	...
	internal static class Patch_MakeNewToils
	{
		private static MethodBase TargetMethod()
		{
			return typeof(JobDriver_ConstructFinishFrame)
				.GetNestedTypes(AccessTools.all).FirstOrDefault(name => name.Name.Contains("DisplayClass8"))
				.GetMethods(AccessTools.all).FirstOrDefault(method => method.Name.Contains("b__1"));
		}
