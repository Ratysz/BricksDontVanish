...
namespace RTBricksDontVanish
{
	...
	internal static class Patch_GBRLC_FailConstruction
	{
		...
		{
            foreach (var nested in typeof(GenLeaving).GetNestedTypes(AccessTools.all))
            {
                foreach (var method in nested.GetMethods(AccessTools.all))
                {
                    if (method.Name.Contains("b__11_6"))
					{
						return method;
					}
                }
	...
	internal static class Patch_GBRLC_Deconstruct
	{
		private static MethodBase TargetMethod()
		{
			return typeof(GenLeaving).GetNestedTypes(AccessTools.all).FirstOrDefault(nested => nested.Name.Contains("DisplayClass11_0"))
